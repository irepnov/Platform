...
namespace SimRodnNom
{
    public partial class frmSootvet : Form
    {
        ...
        private string _proizv = "не задан";
        ...
        private void CreateGrid()
        {
            ...
            dataGridView1.Columns[1].HeaderText = "Родное наименование товара";
            ...
            dataGridView1.Columns[2].HeaderText = "Соответствующее эталонное наименование товара";
        ...
        private void FillEtalon(string inModel)
        {
            if (!inModel.Contains(_proizv)) //если модель от другого производителя, то выбираю список Эталонных моделей
            {
                ...
                _dbsql.SQLScript = "exec uchRodnNomenc @IDOperation = 6, @isIntellect = @ch, @proizv = @pr";
        ...
        private bool UserCheck()
        {
            /* bool _result = false;
             ...
             catch (Exception ex)
             {
                 foreach (DataGridViewRow drv in dataGridView1.Rows)
                 {
                     _dbsql.SQLScript = @"update tmpRodnNomen 
                                          set EtalonName = null,
                                              EtalonNomenRef = null
                                          where id = @id";
        ...
        {
            _dbsql.SQLScript = "select distinct name from tmpRodnNomen where EtalonName is null";
            ...
            try {
                ...
                _excel.SetValueToRange("A1", "Список несопоставленной родной номенклатуры");
                ...
                _dt = null;
                //сохраняем выходим           
                if (_excel.SaveDocument("c:\\temp\\uchNotSopostRodnNom__" + DateTime.Now.Date.ToShortDateString().Replace(".", "_") + ".xls", true, ESaveFormats.xlNormal))
                ...
                if ((drv.Cells[2].Value != null) && (drv.Cells[2].Value.ToString() != ""))
                {
                    _dbsql.SQLScript = @"update tmpRodnNomen 
                                             set EtalonName = e.name,
                                                 EtalonNomenRef = e.id
                                             from tmpRodnNomen t
                                              inner join uchEtalonNomen e on e.Name = @et
                                             where t.id = @id";
        ...
        private void AddNew(string inEtalonName)
        {
            foreach (DataGridViewRow drv in dataGridView1.Rows)
            {
                (drv.Cells[2] as DataGridViewComboBoxCell).Items.Add(inEtalonName);
            }
        }

        private bool AddBaseNew(string inEtalonName)
        {
            try
            {
                _dbsql.SQLScript = @"if not exists(select id from uchEtalonNomen where Name = @name1)
                                    begin
                                        insert into uchEtalonNomen(Name, isActive) values(@name2, 0)
                                    end";
                _dbsql.AddParameter("@name1", EnumDBDataTypes.EDT_String, inEtalonName);
                _dbsql.AddParameter("@name2", EnumDBDataTypes.EDT_String, inEtalonName);
                int i = _dbsql.ExecuteNonQuery();
                return i > 0;
            }
            catch
            {
        ...
        private void btAddEtalon_Click(object sender, EventArgs e)
        {
            DataGridViewRow drv = dataGridView1.CurrentRow;
            String ip = Microsoft.VisualBasic.Interaction.InputBox("Введите наименование", "Создание эталонной номенклатуры", drv.Cells[1].Value.ToString());
            if (ip != "")
            {
                if (AddBaseNew(ip)) AddNew(ip);
            }            
        }
    }
}
